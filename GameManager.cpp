...
GameManager::GameManager()
    ...
    , mStartHover(false)
    , mHighScore(0) // <<< KHỞI TẠO HIGH SCORE
{
...
void GameManager::setupTexts()
{
    ...
    // --- THÊM MỚI: SETUP HIGH SCORE TEXT ---
    mHighScoreText.setFont(mFont);
    mHighScoreText.setCharacterSize(24);
    mHighScoreText.setFillColor(sf::Color::Yellow);
    mHighScoreText.setString("High Score: 0"); 
    sf::FloatRect hsRect = mHighScoreText.getLocalBounds();
    mHighScoreText.setOrigin(hsRect.left + hsRect.width, hsRect.top); 
    mHighScoreText.setPosition(mWindowWidth - 10.f, 10.f); 
    // ----------------------------------------
...
void GameManager::update(sf::Time dt)
{
    ...
    // KHÔNG CẦN CẬP NHẬT HIGH SCORE Ở ĐÂY VÌ NÓ CHỈ THAY ĐỔI KHI GAME OVER HOẶC LOAD
...
void GameManager::render()
{
    ...
    // --- CẬP NHẬT VÀ VẼ HIGH SCORE TEXT (Chung cho mọi trạng thái) ---
    string hsString = "High Score: " + to_string(mHighScore);
    mHighScoreText.setString(hsString);
    sf::FloatRect hsRect = mHighScoreText.getLocalBounds();
    mHighScoreText.setOrigin(hsRect.left + hsRect.width, hsRect.top); // Căn lề lại (Đảm bảo độ chính xác)
...
void GameManager::checkCollisions()
{
    ...
    for (const auto& enemy : mEnemies)
    {
        ...
        if (playerHitbox.intersects(enemyHitbox))
        {
            ...
            // --- CẬP NHẬT HIGH SCORE TRƯỚC KHI LƯU ---
...
void GameManager::loadScoreboard()
{
    ...
    if (!file.is_open())
    {
        mHighScore = 0; // Không có file, High Score là 0
        return;
    }
    ...
    // Đảm bảo mHighScore được khởi tạo đúng sau khi load.
    mScoreboard.clear();
    mHighScore = 0; 
    ...
    // High Score là phần tử đầu tiên trong multimap (nhờ std::greater<int>)
    if (!mScoreboard.empty())
    {
        mHighScore = mScoreboard.begin()->first;
    }
